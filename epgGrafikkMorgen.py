...
def main(kanal = "NRK 2"):
	...
	d = database()
	c = d.cursor()
	sql = """select tid, tittel, tekstekode from epgHKmorgen where sendedato > now() and kanal = %s order by sort ;"""
	...
	sql = """select date(sendedato) from epgHKmorgen where sendedato > now() and kanal = %s order by sort limit 1;"""
