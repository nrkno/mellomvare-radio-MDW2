...
streambaseUrl = 'http://maweb12.intern.nrk.no/scripts/sendEncoderScript.cfm'
streambaseUrl = 'http://maweb12.felles.ds.nrk.no/scripts/sendEncoderScript.cfm'
...
def sendDataStreambase(dok,baseUrl,kanalnavn,interval=4,command='start',password='nettradioscript'):
	#For at den ikke skal sende noe nŒ
	...
	for kanal in kanaler:
		...
		for kanal in distriktskanaler:
			for blokkId in blokker:
				...
				#f.close()
				if verbose:print "Dette svarte streambase:" , u
...
if __name__=='__main__':
	lagMetadata(kanal='gull',datatype='iteminfo')	
