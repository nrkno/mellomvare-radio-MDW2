...
ikkeDls = ['nett','aacstream'] #Legg inn bloknavn som ikke støtter dls teknologien, nettradioen f. eks.
...
def byttTegn(s):
	"Bytter tegn som ikke går på dab"
	u = []
	for i in s:
		i = i.replace('ö','o')
		i = i.replace('ô','o')
...
def hentProgrammeinfo(d, kanal, hovedkanal, distriktssending  =False, style='enkel', useTimeLimit = True, harDistrikter = False, forceDistrikt = True):
	...
	sql = """select tittel, beskrivelse, beskrivelseAlt, artist, tid FROM iteminfo 
	...
	if c.rowcount:
		iProgramme = True
		if verbose:print "%s har et PROGRAM" % kanal
	else:
		iProgramme = False
		if verbose:print "%s IKKE eget PROGRAM" % kanal
	...
	sql = """SELECT tittel, beskrivelse, beskrivelseAlt, artist, tid FROM iteminfo WHERE kanal=%s AND type='programme' AND localid = '1' 
	LIMIT 1;"""
				
	c.execute(sql,(kanal,))
	try:
		try:
			tittel, beskrivelse, beskrivelseAlt, artist, tid = c.fetchone()
		finally:
			c.close()
	
	except TypeError:
		if verbose:print "Feil i utlesing av kanaldata=%s, henter fra hovedkanal=%s" % (kanal, hovedkanal)
...
def hentProgrammeNext(d,kanal,hovedkanal,distriktssending=False):
	"Henter informasjon om det neste programmet som skal på lufta, returnerer en liste med et element."
	c= d.cursor()
	#Dersom det gjeldene programmet er utløpt skal vi altid vise neste
	...
	if type(tid)!=type(''):
		#Patsh for ulike databasedrivere
		tid = tid.isoformat()
	#print tid,tittel
...
def hentIteminfo(d,kanal,hovedkanal,distriktssending=0):
	...
	#TODO
	#Filtrere distriktskanaler som har egene data
	#Flytte blokk ti selve utsendingen
...
def tilDab(kanal='alle',datatype=None,id=''):
	...
	for kanal in kanaler:
		...
		if len(distriktskanaler) == 1:
			...
			hovedkanal = kanal
			harDistrikter = True
			hovedkanalVisning = hentVisningsvalg(d, kanal, datatype=datatype)
			if verbose:print 555, hovedkanalVisning, kanal
		for kanal in distriktskanaler:
			...
			if hentProgrammeinfo(d, kanal, hovedkanal, harDistrikter = harDistrikter)[0][0]=='VOID':
				...
				continue
			if not harDistrikter:
				visningsvalgBlokk = hentVisningsvalg(d, kanal, datatype=datatype)
			else:
				visningsvalgBlokk = hovedkanalVisning
				visningsvalgBlokkLokal = hentVisningsvalg(d, kanal, datatype=datatype)
			if verbose:print '\nSkal vises:', kanal, visningsvalgBlokk.keys() 
			
			#Flush testfilene
			if datatype == 'iteminfo' and not (breaking or testModus):
			...
			for blokk in visningsvalgBlokk:
				if blokk in ikkeDls:
					if verbose:print "Ikke vis som DLS på %s" % blokk
					continue
				#Bygge opp visningslista

				#Filtrere visningsblokker
				if harDistrikter and blokk not in visningsvalgBlokkLokal:
					continue
				...
				if verbose:print 888, visningsvalg, visningsvalgBlokk
				...
				if 'newsItem' in visningsvalg or '1' in kanal:
					...
					s.extend(linja)
					sk.extend(linja)
				if 'newsInfo' in visningsvalg or '1' in kanal:
					#Tillegsinfo om news
...
if __name__=='__main__':
	print tilDab(kanal='alle',datatype='iteminfo')
